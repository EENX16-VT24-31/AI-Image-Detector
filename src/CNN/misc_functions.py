"""
Most of these functions are implpemented by:
...
def save_image(im, path):
    ...
    Args:
        im (Numpy array): Array of shape (D, W, H) or (W, H, D)
        path (str): Path to the image
    """
    print("Image shape before processing:", im.shape)
    ...
    # Adjust for RGB images
    if im.shape[0] == 3:  # Channel-first format (assuming RGB)
        im = im.transpose(1, 2, 0)  # Change to channel-last format (H, W, C) for PIL
        mode = 'RGB'
    elif im.shape[0] == 1:  # Single-channel image, grayscale
        im = im.squeeze(0)  # Remove channel dimension, resulting in (H, W)
